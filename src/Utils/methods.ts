...
export const courseSorting = (
  ...
  switch (sortValue) {
    ...
    case "popular": {
      const sorted = cousesCopy.sort((a, b) => {
        const totalStudentsA = a.total_students ?? 0;
        const totalStudentsB = b.total_students ?? 0;
        return totalStudentsA > totalStudentsB ? -1 : 1;
      });
      setSort(sorted);
      break;
    }
...
export const minutesTosaat = (minutes: number): string => {
  const saat = Math.floor(minutes / 60);
  const minutesLeft = minutes % 60;

  const saatString = saat > 0 ? `${saat}h ` : "";
  const minutesString = minutesLeft > 0 ? `${minutesLeft}m` : "";

  return `${saatString}${minutesString}`;
};
...
export const flatDeep = <T>(arr: any[], d = 1): T[] => {
  return d > 0
    ? arr.reduce((acc, val) => {
        ...
        return acc.concat(Array.isArray(val) ? flatDeep<T>(val, d - 1) : val);
      }, [])
    : arr.slice();
};
